...
class HospitalTests(TestCase):
    ...
    def test_surgeries_on_10_apr_2005_starting_before_noon(self):
        ...
        self.assertListEqual(
            list(qs), list(user_queries.surgeries_on_10_apr_2005_starting_before_noon())
        )

    def test_baileys_surgeries(self):
        qs = Surgery.objects.filter(doctors__last_name="Bailey")
        self.assertListEqual(list(qs), list(user_queries.baileys_surgeries()))

    def test_cardiothoracic_surgeries(self):
        qs = Surgery.objects.filter(doctors__speciality="CAR")
        self.assertListEqual(list(qs), list(user_queries.cardiothoracic_surgeries()))

    def test_shepherds_patients(self):
        qs = Patient.objects.filter(surgery__doctors__last_name="Shepherd").distinct()
        self.assertListEqual(list(qs), list(user_queries.shepherds_patients()))

    def test_number_deceased_patients(self):
        num = Patient.objects.filter(survived=False).count()
        self.assertEqual(num, user_queries.number_deceased_patients())
    ...
    def test_number_of_diagnosis_jerry_frost(self):
        ...
        self.assertEqual(num, user_queries.number_of_diagnosis_jerry_frost())

    def test_earliest_birth_year_of_doctors(self):
        dict = Doctor.objects.aggregate(Min("birth_year"))
        self.assertEqual(dict, user_queries.earliest_birth_year_of_doctors())

    def test_largest_number_of_diagnoses(self):
        dict = Patient.objects.annotate(num_diagnoses=Count("diagnosis")).aggregate(
            Max("num_diagnoses")
        )

        self.assertEqual(dict, user_queries.largest_number_of_diagnoses())
    ...
    def test_average_duration_all_surgeries(self):
        ...
        self.assertEqual(dict, user_queries.average_duration_all_surgeries())
    ...
    def test_surgeries_longer_3hours(self):
        ...
        self.assertListEqual(list(qs), list(user_queries.surgeries_longer_3hours()))
